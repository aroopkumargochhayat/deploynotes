		...
		<div class="panel panel-default" data-collapsed="0">
			<div class="panel-heading">
				<div class="panel-title" >
					<i class="entypo-plus-circled"></i>{{_lang('Add New Material')}}
				</div>
			</div>
			<div class="panel-body">
				<div class="col-md-10">
					<form action="{{route('assignments.store')}}" class="form-horizontal form-groups-bordered validate" autocomplete="off" enctype="multipart/form-data" method="post" accept-charset="utf-8">
						...
						<div class="form-group">
							<label class="col-sm-3 control-label">{{_lang('Course')}}</label>
							<div class="col-sm-6">
						...
						<div class="form-group">
							<label class="col-sm-3 control-label">{{_lang('Branch')}}</label>
							<div class="col-sm-6">
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								<select name="subject_id" class="form-control select2" onChange="getChapters(this.value);" required>
									<option value="">{{ _lang('Select One') }}</option>
								</select>
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-3 control-label">{{_lang('Chapter')}}</label>
							<div class="col-sm-6">
...
<script type="text/javascript">
	function getData(val) {
		...
		$.ajax({
			type: "POST",
			url: "https://notes.dorabhai.com/sections/section",
		...
		$.ajax({
			type: "POST",
			url: "https://notes.dorabhai.com/subjects/subject",
			data:{_token:_token,class_id:class_id},
		...
		var _token=$('input[name=_token]').val();
		var class_id=$('select[name=subject_id]').val();
		$.ajax({
			type: "POST",
			url: "https://notes.dorabhai.com/chapters/chapter",
			data:{_token:_token,subject_id:class_id},
			success: function(sections){
				$('select[name=chapter_id]').html(sections);				
			}
		});
	}
