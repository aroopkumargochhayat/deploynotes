		...
		<div class="panel panel-default" data-collapsed="0">
			<div class="panel-heading">
				<div class="panel-title" >
					<i class="entypo-plus-circled"></i>{{_lang('Update Material')}}
				</div>
			</div>
					...
					<form action="{{route('assignments.update',$assignment->id)}}" class="form-horizontal form-groups-bordered validate" autocomplete="off" enctype="multipart/form-data" method="post" accept-charset="utf-8">
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								<select name="chapter_id" class="form-control select2" required>
									<option value="">{{ _lang('Select One') }}</option>
									{{ create_option('chapters','id','chapter_name',$assignment->chapter_id) }}
								</select>
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<div class="col-sm-offset-3 col-sm-5">
								<button type="submit" class="btn btn-info">{{ _lang('Update Material') }}</button>
							</div>
						</div>
					</form>
				</div>
